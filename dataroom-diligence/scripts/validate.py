...
Failures are dropped and counted. They are never repaired -- a citation that
cannot be verified is not a citation.
"""
...
def check_finding(
    ...
    # The citation includes a page number, so verify that too when ingest.py
    # left a page map behind.
    if page_spans:
        expected = page_for_offset(page_spans, finding.char_start)
        if expected != finding.page:
            return False, f"page_mismatch:cited_{finding.page}_actual_{expected}"
    return True, "ok"
